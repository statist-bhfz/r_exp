...
В книге Applied Predictive Modeling показан лучший результа RMSE < 0.6 на тестовых данных. Попробуем улучшить модель путем более тонкой настройки, не используя более пакет **caret**.
...
4. `subsample` - доля объектов обучающей выборки, используемых на каждой итерации. Еще один критически важный параметр, позволяющий значительно улучшить модель. По умолчанию равен 0, значение 0.5 может оказаться хорошим выбором (но нужно принимать во внимание размер выборки).
...
Снова меняем знак, получаем вторую производную $p(1-p)$. Это в точности те формулы, которые используется в рассматриваемом примере:
